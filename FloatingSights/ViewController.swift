...
class ViewController: UIViewController, ScrollViewDelegate, ARSCNViewDelegate {
    ...
    @IBOutlet var letsFloat: UIButton!
    ...
    // MAEK: - Menu View
    @IBAction func menuButtonTapperd(_: UIButton) {
        toggleMenu()
    }
    ...
    /**
     ボタンイベント
     */
    @objc func onClickMyButton(sender _: UIButton) {
    ...
    @objc func onClickFloat(sender _: UIButton) {
        isFloat = true
        print(isFloat)
    }

    @objc func outClickFloat(sender _: UIButton) {
        isFloat = false
        print(isFloat)
    }
    ...
    func initializeMenu() {
        ...
        for i: Int in 0 ..< imageNames.count {
            let titelLabel = UILabel(frame: CGRect(x: 0, y: 0, width: 200, height: 50))
...
//        }
    ...
    func getRandomNumber(Min _Min: Float, Max _Max: Float) -> Float {
        (Float(arc4random_uniform(UINT32_MAX)) / Float(UINT32_MAX)) * (_Max - _Min) + _Min
    }
}
